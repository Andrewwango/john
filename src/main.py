...
''' 
This is the main program which drives the robot in a simple zig-zag pattern along a pavement,
turning at detected walls and at pavement edges.
The robot can be left to run alone, and also controlled by an IR remote.

The robot orientation is controlled by compass.

More details on the video submission: https://youtu.be/SZ_vZ9dfDFE
...
- Drive until US1 detects object.
	- Activate the higher US (US2)'s position
		- If object not detected, then object is small (litter). Perform litter-picking procedure.
...
#Setup RPi and BrickPi interfacing
...
adjusted = 'no'
debouncetimestamp = time.time() #for manual interrupt
previoususreading = 100
abandonship = False #hard reset flag
...
try:
	...
	def buzz(patternofbuzz):
		...
		for i in range(len(patternofbuzz)):
			...
			time.sleep(0.1 if patternofbuzz[i] == "short" else 0.3)
	...
	def takeusreading(trig, echo, repeats=3, disregardhigh=False):
		...
		uslist=[]
		durationcutoff = DISTANCECUTOFF/34000
		
		for i in range(repeats):
			distance = 0; duration = 0
			#Send out US signal from TRIG
			GPIO.output(trig, False); time.sleep(0.001)
			GPIO.output(trig, True);  time.sleep(0.001)
			GPIO.output(trig, False)
			
			#find length of signal from ECHO
			...
			# NB: breaks when cut off point is bypassed - this prevents stalling.
		...
		uslist.sort(); usreading = uslist[int(len(uslist)/2)] #median (get rid of anomalies)
		previoususreading = usreading
		GPIO.output(trig, False)
		print "US reading is ", str(usreading), uslist #debug
		return usreading
	
	#Read motor position from built in encoder
	def takeencoderreading(port):
		for i in range(3): #in case of encoder glitches, encoder read is fast anyway
			result = BrickPiUpdateValues()
			if not result: #sensor read is successful
	...
	#LITTER PICKING PROCEDURE - see accompanying video for demonstration
	def pickprocedure():
	...
	#TURNING PROCEDURE - see accompanying video for demonstration
	def turnprocedure():
	...
	def movewhilecondition(formofmovement, trig, echo, op, val, power, wallprevention=False, llote=False, disregardhigh=False):
		...
		#Keep checking sensors (wall, cliff, time)
	...
	#DETECTION PROCEDURE - see accompanying video for demonstration
	def detectprocedure(alreadyturning):
		...
		if tempreading < USSTANDARD:
			...
			if takeusreading(US2TRIG, US2ECHO) > US2STANDARD:
				...
				#if while turning, turn back a bit until it's in sight, then make verify
				messedup = True
				...
				if alreadyturning == True:
					for i in range(2):
						...
						if takeusreading(USNEWTRIG,USNEWECHO,repeats=7) > USSTANDARD: #messed up
							print "turning back, monumentally failed"
							movewhilecondition("turnback", USNEWTRIG, USNEWECHO, ">", USSTANDARD, SHIFTPOWER, llote=True)
							messedup = True
						
						if messedup == False: break
				
				# Algorithm to correctly position John for detection
				...
				#move forward/backward if litter is not in optimum range to pick up
				adjusted = 'no'; print "checking adjust"
				...
				if tempreading <= OPTLITTERRANGE[0]: #too close
					print "too close, adjusting AWAY"
					movewhilecondition("backwards", USNEWTRIG, USNEWECHO, "<", OPTLITTERRANGE[0], WHEELPOWER, disregardhigh=True)
					adjustedenc = abs(takeencoderreading(LWHEEL) - startpos) #so we know where we've gone
					adjusted='away'
				
				elif tempreading >= OPTLITTERRANGE[1]: #too far
					print "too far, adjusting TOWARDS"
					movewhilecondition("forwards",  USNEWTRIG, USNEWECHO, ">", OPTLITTERRANGE[1], WHEELPOWER, disregardhigh=True)
					adjustedenc = abs(takeencoderreading(LWHEEL) - startpos)
					adjusted='near'

				#If the stars align, start to PICK UP THE LITTER
				if abandonship == False:
					if timelimitreached == False:
						pickprocedure()
				
				# Undo positioing algorithm
				#move back if adjusted before
				if abandonship == False:
					if adjusted=='away':
						print "adjusting TOWARDS back to original pos"
						movelimbENC(LWHEEL, WHEELPOWER, adjustedenc, RWHEEL, WHEELPOWER)
					elif adjusted=='near':
						print "adjusting AWAY back to original pos"
						movelimbENC(LWHEEL, -WHEELPOWER, adjustedenc, RWHEEL, -WHEELPOWER)
				...
				if alreadyturning == False:
					...
					#I am already turning so I want to get away from detected wall
					print "turning away from goddamn wall"
					#turn until wall is not in sight in case of stalling
